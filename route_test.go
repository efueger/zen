...
func BenchmarkStaticRouteMatch(b *testing.B) {
	s := &Server{
		routeTree: map[string]*route{},
		contextPool: sync.Pool{
			New: func() interface{} {
	...
	for _, r := range staticRoutes {
		s.Route(r.method, r.path, nil)
	}

	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		s.routeMatch("GET", "/progs/json1.go")
	}
}

func BenchmarkParamRouteMatch(b *testing.B) {
	s := &Server{
		routeTree: map[string]*route{},
		contextPool: sync.Pool{
			New: func() interface{} {
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		s.routeMatch("POST", "/repos/:owner/:repo/statuses/:ref")
	}
}
